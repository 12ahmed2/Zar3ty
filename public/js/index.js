...
async function loadProducts() {
  ...
  els.grid.innerHTML = (productsCache || []).map(p => {
    ...
    const stockTxt = stockNum === null ? `<span class="muted">&nbsp;</span>` : (soldOut ? `<span class="error">Sold out</span>` : `<span class="muted">Stock: ${stockNum}</span>`);
    return `
      <article class="card product">
        ...
        <div class="row" style="justify-content:space-between;align-items:center;">
          ...
          <button class="btn sm" data-add="${p.id}" ${soldOut ? 'disabled' : ''}>${soldOut ? 'Sold out' : 'Add'}</button>
        </div>
      </article>`;
  }).join('');
}
...
document.addEventListener('click', async (e) => {
  ...
  if (addBtn) {
    ...
    try {
      ...
      toast(err?.data?.error || 'Add failed');
    }
    return;
  }
  ...
  if (rm) {
    ...
    try {
      ...
      toast(err?.data?.error || 'Remove failed');
    }
    return;
  }
  ...
  if (e.target.closest && e.target.closest('#btn-checkout')) {
    if (typeof window.me === 'undefined' || !window.me) { toast('Log in to checkout'); show(document.querySelector('#dlg-login')); return; }
    try {
      const out = await api('/api/checkout', { method: 'POST' });
      toast(`Order #${out.order_id} placed`);
      await renderCart();
    } catch (err) {
...
document.addEventListener('submit', async (e) => {
  ...
  if (e.target.matches && e.target.matches('#form-login')) {
    ...
    try {
      ...
      if (els.loginErr) els.loginErr.textContent = err?.data?.error || 'Login failed';
    }
    return;
  }
  ...
  if (e.target.matches && e.target.matches('#form-signup')) {
    ...
    try {
      ...
      if (els.signupErr) els.signupErr.textContent = err?.data?.error || 'Signup failed';
    }
    return;
  }
});
